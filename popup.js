...
function bdQueryOk(response) {
    ...
    var learning_id = response.learning_id;
    var voc = response.trans_result[0];

    // word and pronouncation
    var content = document.getElementById('content');
    content.innerHTML = voc.dst + ' ';
}
...
function baiduQuery(word) {
    // show this let user don't panic
    showTips('查询中...');
    clearArea('jump');
    clearArea('definition');

    var appid = '20161206000033541';
    var key = '1jo60WSANmevjlNJM2qD';
    ...
    var request = new XMLHttpRequest();
    var query_url = 'http://api.fanyi.baidu.com/api/trans/vip/translate?q=' + word + '&appid=' + appid + '&salt=' + salt + '&from=' + from + '&to=' + to + '&sign=' + sign;
    request.open('GET', query_url);
    request.onreadystatechange = function() {
        if (request.readyState === 4) {
...
function keydown() {
    if (event.keyCode == 13) {
        ...
        if (word == null) {
            ...
            clearArea('jump');
            clearArea('definition');
            showTips('<span class="color">转百度翻译英文字符</span>和<span class="color">空格</span>为有效的关键字，请重新输入');
        } else
            query(word);
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    //document.querySelector('input').addEventListener('input', change);
